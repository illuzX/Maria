...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT,PICS
...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.edit(script.START_TXT.format(message.from_user.mention if message.from_user else message.chat.title, temp.U_NAME, temp.B_NAME), reply_markup=reply_markup)
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
            	...
            	btn.append([InlineKeyboardButton(" 🔄 Re Try", callback_data=f"{pre}#{file_id}")])
            except IndexError:
                btn.append([InlineKeyboardButton(" 🔄 Re Try", url=f"https://t.me/{temp.U_NAME}/{message.command[1]}")])
    ...
    if settings is not None:
        buttons = [
            ...
            [
                ...
                InlineKeyboardButton(
                    '✅ ON' if settings["welcome"] else '❌ No',
                    callback_data=f'setgs#welcome#{settings["welcome"]}#{grp_id}',
                ),
            ],
        ]
...
async def save_template(client, message):
    ...
    elif chat_type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROU]:
        grp_id = message.chat.id
        title = message.chat.title
